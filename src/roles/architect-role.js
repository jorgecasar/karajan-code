...
export class ArchitectRole extends BaseRole {
  ...
  async execute(input) {
    ...
    const prompt = await buildArchitectPrompt({ task, instructions: this.instructions, researchContext, productContext: this.config?.productContext || null, domainContext: this.config?.domainContext || null });
